...
async function runScript(script: string, workspace: string, args: Record<string, string | undefined>): Promise<string> {
    ...
    const argString = Object.entries(allArgs)
        .filter(([_, v]) => v !== undefined)
        .map(([k, v]) => `-${k} "${v}"`)
        .join(" ");
    
    const { stdout, stderr } = await execAsync(
        `powershell -ExecutionPolicy Bypass -File "${scriptPath}" ${argString}`,
        { encoding: "utf8" }
    );
...
export default function (pi: ExtensionAPI) {
    ...
    pi.registerCommand("task", {
        ...
        async handler(args, ctx) {
            ...
            const task = await findTask(workspace, args);
            if (task) {
                if (task.folder === "Active") {
                    ctx.ui.notify(`"${task.title}" is already active`, "info");
                    return;
                }

                // Auto-switch: move current active to Backlog, set new task to Active
                const activeTasks = await listTasks(workspace, "Active");
                if (activeTasks.length > 0) {
                    await runScript("move-task", workspace, { Name: activeTasks[0].title, NewFolder: "Backlog" });
                }
                await runScript("move-task", workspace, { Name: task.title, NewFolder: "Active" });
                ctx.ui.notify(`Now active: "${task.title}"`, "success");
                return;
            }
            ...
            for (const result of allTasks) {
                for (const t of result.tasks) {
                    if (result.folder !== "Active") {
                        available.push({
                            value: `${result.folder}:${t.title}`,
                            label: `[${result.folder}] ${t.title}`
                        });
                    }
                }
            }
            ...
            const choice = await ctx.ui.select(`No task matching "${args}". Select one:`, available.map(i => i.label));
            if (!choice) {
                ctx.ui.notify("Cancelled", "info");
                return;
            }

            const selected = available.find((_, i) => available[i].label === choice);
            if (selected) {
                const [folder, title] = selected.value.split(":");
